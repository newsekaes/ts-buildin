type StringToUnion<T> = T extends `${infer First}${infer Rest}` ? First | StringToUnion<Rest> : never

type Combine<T extends string> = [T] extends [never] ? "" : "" | {
  [P in T]: `${P}${Combine<Exclude<T, P>>}`
}[T]

type AllCombinations<S extends string> = Combine<StringToUnion<S>>
