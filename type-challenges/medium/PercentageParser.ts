...
type EndParse<A extends string, K extends string[]> =
  A extends `${infer Rest}%`
    ? [...K, Rest, "%"]
    : [...K, A, ""]

type PercentageParser<A extends string, K extends string[] = []> =
  A extends `${"-"|"+"}${infer Rest}`
    ? EndParse<Rest, [ A extends `${infer First}${infer Other}` ? First : never]>
    : EndParse<A, [""]>
